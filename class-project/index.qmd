...
The full PRAMS analytic dataset included 237,392 respondents. After excluding observations with missing values for the primary exposure, outcome, and selected covariates, the final analytic sample included 233,213 respondents. Complete case analysis was used, which may introduce bias if data are not missing completely at random (MCAR). Respondents with missing values may differ systematically from those included, which could affect generalizability.

```{r}#| label: tbl-study-pop
